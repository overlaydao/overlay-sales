...
impl<S: HasStateApi> State<S> {
    ...
    pub(crate) fn whitelisting(&mut self, user: &Address, prior: Prior) {
        self.participants
            .entry(*user)
            .or_insert_with(|| UserState::new(prior, ContractTokenAmount::from(0), TARGET_UNITS));
    }
...
impl<S: HasStateApi> PartialEq for State<S> {
    fn eq(&self, other: &Self) -> bool {
        if self.usdc_contract != other.usdc_contract {
            return false;
        }
...
impl SaleInfo {
    ...
    pub(crate) fn calc_price_per_unit(&self) -> Result<ContractTokenAmount, CustomContractError> {
        // Price_per_unit must not exceed 18_446_744_073_709_551_615
...
mod tests {
    ...
    fn test_state() {
        ...
        assert_eq!(
            ...
            UserState {
                prior: Prior::TOP,
                deposit_usdc: ContractTokenAmount::from(0),
                tgt_units: 1,
                win_units: 0,
                claimed_inc: 0
            },
        ...
        let _ = state.deposit(&USER1_ADDR, ContractTokenAmount::from(100_000_000), 1);
        ...
        assert_eq!(
            ...
            UserState {
                prior: Prior::TOP,
                deposit_usdc: ContractTokenAmount::from(100_000_000),
                tgt_units: 1,
                win_units: 1,
                claimed_inc: 0
            },
    ...
    #[test]
    fn test_divide() {
        let price_per_token: u64 = 2_000_000;
        ...
        claim_eq!(
            price,
            ContractTokenAmount::from(price_per_token * token_per_unit),
