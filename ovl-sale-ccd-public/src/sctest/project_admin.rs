...
mod tests {
    ...
    #[concordium_test]
    /// Test that setTGE successfully set schedule.vesting_start.
    fn test_set_tge() {
        let mut state_builder = TestStateBuilder::new();
        let admin = AccountAddress([0u8; 32]);
        let proj_admin = AccountAddress([1u8; 32]);
        ...
        let expected_state = State {
            ...
            schedule: SaleSchedule {
                open_at: open_at.clone(),
                close_at,
                vesting_start: Some(vesting_start_to_be_set),
                vesting_period: vesting_period.clone(),
            },
            saleinfo: SaleInfo {
                price_per_token,
        ...
        // execute func
        let result = contract_set_tge(&ctx, &mut host);
        claim!(result.is_ok());
        claim_eq!(
            *host.state(),
            expected_state,
    ...
    #[concordium_test]
    /// Test that projectClaim successfully transfers all CCD to admin address.
    fn test_project_claim() {
        let mut state_builder = TestStateBuilder::new();
        let amount_to_claim = Amount::from_ccd(100);
        let admin = AccountAddress([0u8; 32]);
        let proj_admin = AccountAddress([1u8; 32]);
        let project_token_address = ContractAddress {
            index: 1000,
        ...
        let mut host = TestHost::new(initial_state, state_builder);
        host.set_self_balance(amount_to_claim);
